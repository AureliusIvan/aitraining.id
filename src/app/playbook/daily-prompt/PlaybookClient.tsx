...
const anatomyElements = [
  {
    ...
    example: 'Kamu adalah seorang copywriter senior dengan 10 tahun pengalaman di industri FMCG.',
    color: 'text-blue-400',
  ...
  },
  {
    ...
    example: 'Perusahaan kami baru launching produk skincare untuk remaja usia 15-20 tahun.',
  ...
  },
  {
    ...
    example: "Jangan pakai bahasa yang terlalu formal. Tone: playful, relatable, Gen-Z friendly.",
    color: 'text-rose-400',
...
const templates = [
  {
    ...
    description: 'Follow-up meeting, minta approval, update ke atasan — email bisnis yang langsung to the point.',
...
- Akhiri dengan clear next step / CTA`,
  },
  {
    ...
    description: 'Dokumen panjang, notulen meeting, artikel — jadi ringkasan terstruktur yang bisa langsung dipakai.',
...
[PASTE DOKUMEN/NOTULEN DI SINI]`,
  },
  {
    ...
    description: 'Generate caption, copy, dan ide konten yang sesuai platform, audiens, dan tujuan kampanye.',
...
- Sesuaikan dengan character limit platform`,
  },
  {
    ...
    description: 'Interpretasi sales report, survey result, atau analytics dalam bahasa yang bisa dipresentasikan.',
...
[PASTE DATA DI SINI — bisa tabel, CSV, atau deskripsi]`,
  },
  {
    ...
    description: 'Stuck? Generate ide dari yang safe sampai bold — lengkap dengan kelebihan, risiko, dan effort level.',
...
- Jangan ulangi ide yang sudah pernah dicoba`,
  },
  {
    ...
    description: 'Polish email, proposal, laporan, atau artikel — grammar, flow, tone, dan struktur sekaligus.',
...
[PASTE TULISAN DI SINI]`,
  },
  {
    ...
    description: 'Dari agenda, talking points, sampai antisipasi pertanyaan sulit — siap masuk ruangan dalam menit.',
...
const tips = [
  { emoji: '🔄', text: 'Hasil jelek? Jangan buang — kasih feedback spesifik ke AI tentang bagian mana yang kurang.' },
  { emoji: '✂️', text: 'Pecah tugas besar jadi langkah kecil. Jangan minta proposal 20 halaman dalam 1 prompt.' },
  { emoji: '📎', text: 'Selalu kasih contoh output yang lo mau. AI jauh lebih akurat kalau ada referensi.' },
  { emoji: '⚠️', text: 'Jangan lupa fact-check. AI bisa confident tapi salah.' },
  { emoji: '💾', text: 'Simpan prompt yang berhasil. Bikin swipe file prompt lo sendiri.' },
  { emoji: '🎯', text: 'Kalau output terlalu generic, tambahin konteks spesifik tentang audiens, industri, atau situasi lo.' },
  { emoji: '🍒', text: 'Minta AI kasih beberapa opsi, bukan cuma satu jawaban. Lo bisa cherry-pick yang terbaik.' },
  { emoji: '🤝', text: 'Pakai AI buat first draft, bukan final output. Sentuhan manusia tetap penting.' },
];
...
function renderTemplate(text: string) {
  return text.split(/(\[[^\]]+\])/g).map((part, i) =>
    /^\[[^\]]+\]$/.test(part) ? (
      <mark key={i} className="bg-amber-400/15 text-amber-300 rounded px-0.5 not-italic">
        {part}
      </mark>
    ) : (
...
function CopyBtn({ text, label = 'Copy Prompt' }: { text: string; label?: string }) {
...
function TemplateCard({
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-xl w-8 text-center flex-shrink-0 select-none">{t.icon}</span>
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-white text-base">{t.title}</p>
          <p className="text-sm text-white/50 mt-0.5 leading-snug">{t.description}</p>
        </div>
        <svg
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="pt-24">
        {/* ── HERO ──────────────────────────────── */}
        <section id="hero" className="px-6 sm:px-8 py-16 border-b border-white/10">
          ...
          <div className="flex gap-12 py-14">
            ...
            <aside className="hidden lg:block w-52 flex-shrink-0">
              <div className="sticky top-28">
                <p className="text-[10px] text-white/25 uppercase tracking-widest mb-4 font-mono">Sections</p>
                ...
                <p className="text-[10px] text-white/25 uppercase tracking-widest mb-3 font-mono">Templates</p>
            ...
            <div className="flex-1 min-w-0 space-y-20">
              ...
              <section id="anatomy" className="scroll-mt-24">
                <div className="mb-8">
                  <p className="text-white/30 text-xs font-mono mb-2 tracking-widest">[ Section 01 ]</p>
                ...
                <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
                  {anatomyElements.map((el) => (
                    ...
                    >
                      <div className="flex items-center gap-3 mb-4">
                        <span className="font-mono text-xs text-white/25">{el.number}</span>
                        <span className={`text-xl font-bold ${el.color}`}>{el.label}</span>
                      </div>
                      <p className="text-white/55 text-sm mb-4">{el.question}</p>
                      <div className="bg-black/40 border border-white/8 rounded-xl p-3">
                        <p className="text-[10px] text-white/25 font-mono mb-1.5 uppercase tracking-wider">Contoh</p>
                  ...
                  <div className="border border-amber-400/20 bg-amber-400/[0.03] rounded-2xl p-6 sm:col-span-2 xl:col-span-3">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {['Role', 'Context', 'Task', 'Format', 'Constraint'].map((el, i, arr) => (
              ...
              <section id="templates" className="scroll-mt-24">
                <div className="mb-8">
                  <p className="text-white/30 text-xs font-mono mb-2 tracking-widest">[ Section 02 ]</p>
                  <h2 className="text-3xl sm:text-4xl font-bold text-white mb-3">
                ...
                {search === '' && filtered.length > 1 && (
                  <div className="mt-5 flex gap-4">
                    <button
                      onClick={() => setOpen(new Set(templates.map((t) => t.id)))}
              ...
              <section id="tips" className="scroll-mt-24">
                <div className="mb-8">
                  <p className="text-white/30 text-xs font-mono mb-2 tracking-widest">[ Section 03 ]</p>
                  <h2 className="text-3xl sm:text-4xl font-bold text-white mb-3">Tips Cheat Sheet</h2>
                  <p className="text-white/55">
              ...
              {/* ── CTA ─────────────────────────── */}
              <section id="cta" className="scroll-mt-24 border-t border-white/10 pt-16 pb-8">
                <div className="max-w-2xl">
                  <p className="text-white/30 text-xs font-mono mb-4 tracking-widest">[ Mau lebih? ]</p>
                  ...
                  <p className="text-white/65 mb-8 leading-relaxed text-lg">
                    ...
                    . Join webinar lengkapnya untuk belajar cara pakai AI secara optimal di tempat kerja.
                  </p>
                  <div className="flex flex-wrap gap-3">
                    <a
                ...
                <p className="mt-14 text-white/25 text-xs font-mono leading-relaxed">
                  Playbook ini dibuat sebagai companion guide dari webinar &ldquo;Cara Pakai AI Biar Hasilnya Nggak Sampah&rdquo;
                  <br />
                  oleh{' '}
                  <a
                    href="https://aurelivan.com"
